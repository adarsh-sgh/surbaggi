...
socket.on('opponentLeft', () => {
    notice('your opponent left the game');
});
...
canvas.width = 4 * x + 2 * m - 1;
canvas.height = 8 * x + 2 * m - 1; // -1 is bugfix
var ctx = canvas.getContext('2d');
let timeoutId;
let piecePosition;
function resetBoard() {
    piecePosition = [
        //0 invalid pos 1 blue 2 red 3 empty ;
        [2, 0, 2, 0, 2],
        [0, 2, 2, 2, 0],
        [2, 2, 2, 2, 2],
        [2, 2, 2, 2, 2],
...
function boardDraw() {
    ...
    lineDraw(0, 2, 4, 6);
    lineDraw(4, 2, 0, 6); //main diagnols
    lineDraw(1, 2, 1, 6);
    lineDraw(3, 2, 3, 6); //vertical lines
    lineDraw(0, 3, 4, 3);
    lineDraw(0, 5, 4, 5); //Hor lines
    lineDraw(1, 1, 3, 1);
    lineDraw(1, 7, 3, 7); //barrack hor line
    lineDraw(2, 0, 2, 8);
    lineDraw(0, 4, 4, 4); //central line
}
...
function clickHandler(evnt) {
    if (!paired) {
        return notice('Still searching for opponent', 2000);
    }
    if (turnOf == 2) {
        return notice('let your opponent move first', 1000);
    }
    let rect = canvas.getBoundingClientRect();
    let borderWidth = +getComputedStyle(document.getElementById('board'), null)
    ...
    if (piecePosition[fromY][fromX] != 1) {
        return;
    }
    ...
    canReach[String(fromX) + String(fromY)].forEach((item) => {
        ...
        if (
            piecePosition[+item[1]][+item[0]] == 2 &&
            piecePosition[2 * +item[1] - fromY][2 * +item[0] - fromX] == 3
        ) {
...
function pieceMover(evnt, killStreaking = false) {
    ...
    if (
        canReach[String(fromX) + String(fromY)].includes(
    ...
    let canKill =
        piecePosition[(fromY + toY) / 2] &&
        piecePosition[(fromY + toY) / 2][(fromX + toX) / 2] == 2 &&
        piecePosition[toY] &&
        piecePosition[toY][toX] == 3;
    if (canKill) {
        if (killStreaking && timeoutId) {
            clearTimeout(timeoutId);
        }
        piecePosition[fromY][fromX] = 3;
        piecePosition[toY][toX] = turnOf;
        piecePosition[(fromY + toY) / 2][(fromX + toX) / 2] = 3;
        //send kill message
...
function killStreak(evnt) {
    ...
    if (streakMissed) {
        ...
        piecePosition[(yclicked + toY) / 2] &&
        piecePosition[(yclicked + toY) / 2][(xclicked + toX) / 2] == turnOf
    ) {
        turnOf = 3 - turnOf;
        fromX = toX;
        fromY = toY;
...
function executeMessage(p1, p2) {
    ...
    if (
        Number.isInteger(midPointx) &&
        Number.isInteger(midPointy) &&
        pp(`${midPointx}${midPointy}`) == 1
    ) {
